...
export const geminiService = {
  async generateRecommendations(
    ...
    try {
      ...
      return {
        ...
        reasoning:
          "Baseado em suas preferências por filmes de ficção científica e ação.",
      };
    }
  },
  ...
  buildRecommendationPrompt(
    ...
    const experienceText = this.getExperienceDescription(
...
FILTROS APLICADOS:`;
    ...
    if (filters.platforms?.length) {
      ...
      prompt += `\nIMPORTANTE: Recomende apenas títulos que estejam disponíveis nas plataformas selecionadas acima. Não recomende títulos de outras plataformas.`;
    }
...
TAREFA:
Recomende 1 filme/série/anime PRINCIPAL e 8 SUGESTÕES SECUNDÁRIAS, todos baseados no perfil e respostas do usuário. As sugestões devem ser relevantes e variadas, mas sempre conectadas ao que o usuário respondeu no formulário e filtros.

FORMATO DE RESPOSTA (EXATO):
FEATURED: [Nome do título principal]
SUGGESTIONS: [Título1], [Título2], [Título3], [Título4], [Título5], [Título6], [Título7], [Título8]
...
Use apenas títulos conhecidos e populares. Seja preciso no formato da resposta. NÃO repita títulos do histórico.`;
  ...
  parseGeminiResponse(text: string): {
    ...
    try {
      ...
      return {
        ...
        reasoning:
          "Recomendações baseadas em suas preferências cinematográficas.",
      };
    }
  },
  ...
  getExperienceDescription(experience?: string): string {
    ...
    return (
      descriptions[experience as keyof typeof descriptions] ||
      "Experiência variada"
    );
  },
  ...
  getTimeDescription(timeframe?: string): string {
    ...
    return (
      descriptions[timeframe as keyof typeof descriptions] || "Tempo flexível"
    );
  },
  ...
  getCompanyDescription(company?: string): string {
    ...
    return (
      descriptions[company as keyof typeof descriptions] || "Companhia variada"
    );
  },
};
